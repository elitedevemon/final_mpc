<div>
  <div class="app-content main-content pt-3">
    <div class="side-app">
      @php
        #Like related query
        $checklike = App\Models\Backend\FaqsLike::where('username', Auth::user()->username)->where('faq_id', $faq->id)->where('action', 'like')->first();
        $total_like = App\Models\Backend\FaqsLike::where('faq_id', $faq->id)->where('action', 'like')->get();

        #Dislike related query
        $checkdislike = App\Models\Backend\FaqsLike::where('username', Auth::user()->username)->where('faq_id', $faq->id)->where('action', 'dislike')->first();
        $total_dislike = App\Models\Backend\FaqsLike::where('faq_id', $faq->id)->where('action', 'dislike')->get();
      @endphp
      <!-- Row -->
      <div class="row">
          ...
          <div class="card overflow-hidden">
            <div class="card-body">
              ...
              <div class="media py-3 mt-0 border-top">
                ...
                <div class="ms-auto">
                  <div class="d-flex">
                    <a href="javascript:void(0)" title="Like"  wire:click="like('{{ $faq->id }}')" class="new ms-3 mt-2"><i class="p-3 text-{{ $checklike?'primary':'success' }} br-7 bg-{{ $checklike?'primary':'success' }}-transparent fe fe-thumbs-up  fs-16 text-icon"></i></a>
                    <a href="javascript:void(0)" title="Dislike" wire:click="dislike('{{ $faq->id }}')" class="new ms-3 mt-2"><i class="p-3 text-{{ $checkdislike?'danger':'success' }} br-7 bg-{{ $checkdislike?'danger':'success' }}-transparent fe fe-thumbs-down  fs-16 text-icon"></i></a>
                  ...
                  <div class="media-body Comments1">
                    ...
                    <div class="d-flex float-md-end mb-2">
                      @php
                        ...
                        $check_comment_like = App\Models\Backend\FaqCommentLike::where('username', $user->username)->where('faq_id', $faq->id)->where('comment_id', $comment->id)->where('action', 'like')->first();

                        $total_comment_like = App\Models\Backend\FaqCommentLike::where('faq_id', $faq->id)->where('comment_id', $comment->id)->where('action', 'like')->get();

                        #Dislike related queries
                        $check_comment_dislike = App\Models\Backend\FaqCommentLike::where('username', $user->username)->where('faq_id', $faq->id)->where('comment_id', $comment->id)->where('action', 'dislike')->first();

                        $total_comment_dislike = App\Models\Backend\FaqCommentLike::where('faq_id', $faq->id)->where('comment_id', $comment->id)->where('action', 'dislike')->get();
                      @endphp
          ...
          <div class="card mb-lg-0">
            ...
            <div class="card-body">
              <div class="mt-2">
                ...
                <a href="javascript:void(0);" class="btn btn-primary" wire:click="PostComment('{{ $faq->id }}')">
